...
def main():
        ...
        for j in range(square_size):
            drone = scene.add_entity(
                morph=gs.morphs.Drone(
                    ...
                    pos=(i * distance, j * distance, 0.02),
                ),
            )
            drones.append(drone)
    ...
    if args.mac:
        ...
        run_sim(scene, drones, args.vis)
    publisher.spin()


def run_sim(scene, drones, enable_vis):
    traj = np.array(
        [
    ...
    # while True:
    #     drone.set_propellels_rpm((1 + 0.05 * np.array([1.0, 1.0, 1.0, 1.0])) * 14468.429183500699)
    #     scene.step()
