...
def _spaw_func(
    ...
    mesh_obj.apply_transform(trimesh.transformations.euler_matrix(math.pi * 0.5, 0, 0))
...
class FrankaSoftBasketLiftEnvCfg(FrankaTeddyBearLiftEnvCfg):
    def __post_init__(self):
        ...
        self.scene.object = DeformableObjectCfg(
            ...
            init_state=DeformableObjectCfg.InitialStateCfg(pos=(0.5, 0, 0.2), rot=(0.707, 0, 0, 0.707)),
            spawn=MeshTrimeshCfg(
                mesh=f"{Path(__file__).parent.as_posix()}/basket.obj",
                deformable_props=sim_utils.DeformableBodyPropertiesCfg(rest_offset=0.0, contact_offset=0.001),
                visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.5, 0.1, 0.0)),
                physics_material=sim_utils.DeformableBodyMaterialCfg(poissons_ratio=0.4, youngs_modulus=5e4),
            ),
        )

        self.scene.table.spawn.usd_path = f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/thor_table.usd"
        self.scene.table_2 = copy.deepcopy(self.scene.table)
        self.scene.table_2.prim_path = "{ENV_REGEX_NS}/Table2"
        self.scene.table_2.init_state.pos = [0.5, 0.76, 0]

        self.scene.robot.init_state.pos = [0.5, -0.5, 0.0]
