...
from .net_manager import NetManager, Streamer, StrBytesService, init_node
...
class SimPublisher(ServerBase):
    ...
    async def search_xr_device(self, node: NetManager):
        while node.running:
            for xr_info in node.net_info_manager.nodes_info.values():
