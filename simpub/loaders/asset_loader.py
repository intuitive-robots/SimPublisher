...
import simpub
from simpub.simdata import UnityMesh, UnityTexture
import numpy as np   
import trimesh
...
class TextureLoader:
    ...
    @staticmethod
    def fromBuiltin(name : str, builtin_name : str, tint : Optional[np.ndarray] = None) -> Tuple[UnityTexture, bytes]:
        img : Image.Image
...
class MeshLoader:
    @staticmethod
    def from_file(file : Path, name : Optional[str] = None, scale=None) -> UnityMesh: 
        path = Path(file) if isinstance(file, str) else file
        mesh = trimesh.load_mesh(path)
        return MeshLoader.from_loaded_mesh(mesh, name or path.stem, scale)

    @staticmethod
    def from_bytes(name : Path, content : bytes, mesh_type : str, scale : np.ndarray) -> Tuple[UnityMesh, bytes]:
